# 웹 크롤링 (Web Crawling) : 브라우저 드라이버를 이용하여 실제로 각 페이지를 이동하며 '동적'으로 데이터를 수집하는 방법
# 웹 스크래핑 (Web Scrapping) : 웹 페이지의 응답을 받아 데이터를 분석하여 필요한 데이터를 수집하는 방법
...
# 두개 한번에 설치 : pip install beautifulsoup4 selenium
...
# print(response) 결과 : <Response [200]>
print(response.status_code) # 결과 : 200
# print(response.text) 결과 : HTML 코드
...
for news_title_element in news_title_elements:
    ...
    title = news_title_element.get_attribute('title') # 속성
    print(title)
time.sleep(1)
